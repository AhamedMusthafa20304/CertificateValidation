...
export const AuthPage: React.FC<AuthPageProps> = ({ onLogin }) => {
  ...
  const [adminForm, setAdminForm] = useState({ id: 'asdf@gmail.com', password: 'password123', institution: 'abcd' });
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      let name = '', extraId = '';
      if (activeTab === 'ADMIN') {
        if (!adminForm.id || !adminForm.password || !adminForm.institution) { setIsLoading(false); return; }
        name = adminForm.id; extraId = adminForm.institution;
      } else if (activeTab === 'VERIFIER') {
        if (!verifierForm.name) { setIsLoading(false); return; }
        name = verifierForm.name; extraId = verifierForm.org || 'Independent Verifier';
      } else {
        if (!studentForm.id || !studentForm.password) { setIsLoading(false); return; }
        name = studentForm.id; extraId = studentForm.id;
      }
      const user = await blockchainService.login(activeTab, name, extraId);
      onLogin(user);
    } catch (error) { console.error("Login failed", error); } finally { setIsLoading(false); }
  };
  ...
  return (
    <div style={{minHeight:'calc(100vh - 130px)',display:'flex',alignItems:'center',justifyContent:'center',padding:'40px 20px'}}>
      <div style={{width:'100%',maxWidth:'440px',borderRadius:'24px',overflow:'hidden',boxShadow:'0 20px 60px rgba(0,0,0,0.25)',background:roleColors[activeTab]}}>
        ...
        {/* Tabs */}
        <div style={{display:'flex',background:'rgba(0,0,0,0.15)',margin:'0 24px',borderRadius:'12px',padding:'4px',marginBottom:'24px'}}>
          {tabs.map(({role,label,icon}) => (
            <button
              key={role}
              onClick={()=>setActiveTab(role)}
              style={{flex:1,padding:'10px 8px',borderRadius:'10px',border:'none',cursor:'pointer',fontWeight:700,fontSize:'0.85rem',display:'flex',alignItems:'center',justifyContent:'center',gap:'6px',transition:'all 0.2s',
                background:activeTab===role?'rgba(255,255,255,0.9)':'transparent',
                color:activeTab===role?'#c0306a':'rgba(255,255,255,0.8)'
              }}
            >
              {icon}{label}
            </button>
          ...
          <form onSubmit={handleLogin} style={{display:'flex',flexDirection:'column',gap:'14px'}}>
            ...
            {activeTab === 'ADMIN' && (<>
              <div style={{position:'relative'}}>
                ...
                <div style={{position:'relative'}}>
                  ...
                  <input type="text" required value={adminForm.id} onChange={e=>setAdminForm({...adminForm,id:e.target.value})} style={inputStyle} />
                </div>
              </div>
              <div>
                ...
                <div style={{position:'relative'}}>
                  ...
                  <input type="password" required value={adminForm.password} onChange={e=>setAdminForm({...adminForm,password:e.target.value})} style={inputStyle} />
                </div>
              </div>
              <div>
                ...
                <div style={{position:'relative'}}>
                  ...
                  <input type="text" required value={adminForm.institution} onChange={e=>setAdminForm({...adminForm,institution:e.target.value})} style={inputStyle} />
                </div>
              </div>
            </>)}
            ...
            {activeTab === 'VERIFIER' && (<>
              <div>
                ...
                <div style={{position:'relative'}}>
                  <User style={{position:'absolute',left:'14px',top:'50%',transform:'translateY(-50%)',color:'rgba(255,255,255,0.6)',width:'16px',height:'16px'}} />
                  <input type="text" required value={verifierForm.name} onChange={e=>setVerifierForm({...verifierForm,name:e.target.value})} style={inputStyle} />
                </div>
              </div>
              <div>
                ...
                <div style={{position:'relative'}}>
                  <Building2 style={{position:'absolute',left:'14px',top:'50%',transform:'translateY(-50%)',color:'rgba(255,255,255,0.6)',width:'16px',height:'16px'}} />
                  <input type="text" value={verifierForm.org} onChange={e=>setVerifierForm({...verifierForm,org:e.target.value})} style={inputStyle} />
                </div>
              </div>
            ...
            {activeTab === 'STUDENT' && (<>
              <div>
                ...
                <div style={{position:'relative'}}>
                  <User style={{position:'absolute',left:'14px',top:'50%',transform:'translateY(-50%)',color:'rgba(255,255,255,0.6)',width:'16px',height:'16px'}} />
                  <input type="text" required value={studentForm.id} onChange={e=>setStudentForm({...studentForm,id:e.target.value})} style={inputStyle} />
                </div>
              </div>
              <div>
                <label style={{display:'block',color:'rgba(255,255,255,0.9)',fontWeight:600,fontSize:'0.85rem',marginBottom:'6px'}}>Password</label>
                <div style={{position:'relative'}}>
                  <KeyRound style={{position:'absolute',left:'14px',top:'50%',transform:'translateY(-50%)',color:'rgba(255,255,255,0.6)',width:'16px',height:'16px'}} />
                  <input type="password" required value={studentForm.password} onChange={e=>setStudentForm({...studentForm,password:e.target.value})} style={inputStyle} />
                </div>
              </div>
            </>)}
            ...
            >
              {isLoading ? (<><Loader2 style={{width:'18px',height:'18px',animation:'spin 1s linear infinite'}} /> Connecting...</>) : (<>{activeTab==='VERIFIER'?'Access Verifier':'Connect Wallet'} <ArrowRight style={{width:'18px',height:'18px'}}/></>)}
            </button>
          </form>

          <p style={{textAlign:'center',color:'rgba(255,255,255,0.6)',fontSize:'0.75rem',marginTop:'16px'}}>
            By connecting, you agree to our Terms. A mock wallet address will be generated for this session.
          </p>
        </div>
      </div>
    </div>
