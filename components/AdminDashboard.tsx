...
import { Upload, CheckCircle, Loader2, QrCode, History, Copy, Download, MousePointer2, ArrowLeft, Users, FileText, ClipboardList } from 'lucide-react';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ user }) => {
        ...
        if (file.type === 'application/pdf') {
          try {
            ...
            if (canvas) {
              ...
              if (context) await page.render({ canvasContext: context, viewport } as any).promise;
            }
  ...
  const generateFinalPDF = async (cert: StudentCertificate, download = false) => {
    try {
      ...
      if (download) {
        ...
        link.href = url;
        link.download = `${cert.studentName}_Certificate.pdf`;
  ...
  const labelStyle: React.CSSProperties = { display:'block', color:'white', fontWeight:600, fontSize:'0.9rem', marginBottom:'8px' };
  ...
  if (view === 'HOME') {
    return (
      <div style={{minHeight:'calc(100vh - 130px)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'40px 20px'}}>
        <h1 style={{fontSize:'2.5rem',fontWeight:800,color:'white',marginBottom:'48px',textAlign:'center',textShadow:'0 2px 20px rgba(0,0,0,0.15)'}}>Admin Actions</h1>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:'24px',width:'100%',maxWidth:'900px',marginBottom:'40px'}}>
          {[
            {num:'1',icon:<Users style={{color:'white',width:'28px',height:'28px'}}/>,title:'Register New Student',desc:'Create a secure login account and save the student\'s personal profile data.',action:()=>setView('ISSUE')},
            {num:'2',icon:<FileText style={{color:'white',width:'28px',height:'28px'}}/>,title:'Issue Certificate',desc:'Upload the PDF, calculate its hash, and register it on the Ethereum blockchain.',action:()=>setView('ISSUE')},
            {num:'3',icon:<ClipboardList style={{color:'white',width:'28px',height:'28px'}}/>,title:'View All Records',desc:'Browse and manage all registered student and certificate records.',action:()=>setView('HISTORY')},
          ].map((card,i) => (
            <div
              key={i}
              onClick={card.action}
        ...
        <div style={{maxWidth:'900px',width:'100%',background:'rgba(255,255,255,0.12)',border:'2px solid rgba(255,255,220,0.5)',borderLeft:'6px solid #ffd166',borderRadius:'12px',padding:'20px 24px',backdropFilter:'blur(8px)'}}>
          ...
          <p style={{color:'rgba(255,255,255,0.85)',fontSize:'0.9rem'}}>You must <strong>Register a New Student</strong> (Step 1) before you can <strong>Issue a Certificate</strong> (Step 2) for that student.</p>
        </div>
      </div>
    );
  }
            ...
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:'0.9rem'}}>
              <thead>
                <tr style={{background:'linear-gradient(135deg,#f72585,#e85d04)'}}>
                  {['Student','Course','Date Issued','Hash','Action'].map((h,i) => (
                    <th key={i} style={{padding:'16px 20px',color:'white',fontWeight:700,textAlign:i===4?'right':'left',whiteSpace:'nowrap'}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                ...
                {history.length === 0 && (
                  <tr><td colSpan={5} style={{padding:'48px',textAlign:'center',color:'#999'}}>No certificates issued yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
  ...
  return (
    <div style={{maxWidth:'1100px',margin:'0 auto',padding:'40px 20px'}}>
      ...
      <h2 style={{fontSize:'1.8rem',fontWeight:800,color:'white',marginBottom:'24px'}}>Issue Certificate</h2>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{marginBottom:'16px'}}>
              ...
              <input type="text" name="studentName" value={formData.studentName} onChange={handleInputChange}
                style={{width:'100%',padding:'11px 14px',borderRadius:'10px',border:'2px solid #e0e0e0',outline:'none',fontSize:'0.9rem',boxSizing:'border-box'}} placeholder="" />
            </div>
            ...
            <p style={{fontSize:'0.78rem',color:'#888',marginBottom:'16px'}}>Upload a background (Image or PDF). QR code is fixed at X: 137.1mm, Y: 223.4mm.</p>
            ...
            {!templatePreview ? (
              ...
              <div style={{marginBottom:'20px'}}>
                <div ref={previewRef} onClick={handlePreviewClick} style={{position:'relative',border:'2px solid #e0d0ff',borderRadius:'12px',overflow:'hidden',aspectRatio:'210/297',background:'#eee'}}>
                  ...
                  <div style={{
                    position:'absolute',border:'2px solid #7c3aed',background:'rgba(124,58,237,0.2)',
                    display:'flex',alignItems:'center',justifyContent:'center',pointerEvents:'none',
                    left:`${(qrConfig.x/210)*100}%`,top:`${(qrConfig.y/297)*100}%`,
                    width:`${(qrConfig.size/210)*100}%`,height:`${(qrConfig.size/297)*100}%`
                  }}>
            ...
            <button type="submit" disabled={isIssuing} style={{width:'100%',padding:'14px',background:'linear-gradient(135deg,#f72585,#e85d04)',border:'none',borderRadius:'12px',color:'white',fontWeight:800,fontSize:'1rem',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'8px',opacity:isIssuing?0.7:1}}>
              {isIssuing ? <><Loader2 style={{width:'20px',height:'20px',animation:'spin 1s linear infinite'}}/> Minting...</> : 'Mint & Generate Certificate'}
            </button>
          </form>
        </div>
        ...
        <div style={{background:'rgba(255,255,255,0.95)',borderRadius:'20px',padding:'32px',boxShadow:'0 8px 32px rgba(0,0,0,0.15)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'400px'}}>
          {issuedCert ? (
            <div style={{textAlign:'center',width:'100%',maxWidth:'360px'}}>
              ...
              <p style={{color:'#888',fontSize:'0.85rem',marginBottom:'20px'}}>ID: {issuedCert.certificateId}</p>
